// ============================================================
// TechCorp Lab controller (Lab 2 — The Exposed Employee File)
// ============================================================
...
function updateProgress(data) {
  ...
  const completedMissions = [
    mission >= 2, // Mission 1: login
    mission >= 3, // Mission 2: explore documents
    mission >= 4, // Mission 3: inspect file paths / directory listing
    data.flag_captured, // Mission 4: exposed file
...
const fileContents={
 'notes.txt':'Employee document portal investigation\nSarah says internal files are showing up where they should not.\nRemember: being logged in is not the same as being allowed to read every file.',
 'portal-recon.txt':'PORTAL RECON\n\nDocuments load from paths like:\n  /files/EMP1042/profile.txt\n\nThe folder (employee ID) and file name both look predictable.\nQuestion: what happens if you request a folder instead of a file?',
 'reminders.txt':'Investigate how the file server builds paths.\nTry walking up from /files/EMP1042/ to /files/ and see what is listed.\nSensitive files should never be reachable from the employee portal.'
};
...
function openNotes(){if(!requireLab())return;const w=document.getElementById('notes-window');w.classList.add('open');bringToFront(w);const saved=localStorage.getItem('techcorp-lab2-notes');if(saved!==null)document.getElementById('notes-editor').value=saved;}
function saveNotes(){localStorage.setItem('techcorp-lab2-notes',document.getElementById('notes-editor').value);document.getElementById('notes-saved').textContent='Saved';setTimeout(()=>document.getElementById('notes-saved').textContent='',1200);}

const commandHelp='help — show commands\nwhoami — show current analyst\npwd — show current directory\nls — list files\ncat <file> — read a local note\nclear — clear terminal\ncurl <path> — simulated HTTP status for the lab';
...
function handleTerminalKey(e){
  ...
  const input=e.target;const cmd=input.value.trim();input.remove();const safe=cmd.toLowerCase();
  if(safe==='clear'){document.getElementById('terminal-output').innerHTML='Terminal cleared.\n<span class="terminal-prompt">alex@techcorp-analyst:~$</span> <input id="terminal-input" autocomplete="off" spellcheck="false" onkeydown="handleTerminalKey(event)">';document.getElementById('terminal-input').focus();return;}
  let result='Command not found. Type help for available commands.';
  if(safe==='help')result=commandHelp;
  else if(safe==='whoami')result='alex';
  else if(safe==='pwd')result='/home/alex';
  else if(safe==='ls')result='Desktop  Documents  notes.txt  portal-recon.txt  reminders.txt';
  else if(safe.startsWith('cat ')){const name=cmd.slice(4).trim();result=fileContents[name]||'cat: file not found';}
  else if(safe==='curl /files/'||safe==='curl /files')result='HTTP/1.1 200 OK  (directory listing enabled — real requests happen in the Firefox window)';
  else if(safe.startsWith('curl /files/'))result='HTTP/1.1 200 OK  (the file server returns documents by path — try it for real in Firefox + DevTools)';
  else if(safe.startsWith('curl '))result='Use the Firefox browser in the desktop to make real requests to 127.0.0.1:5000.';
...
function updateCompletionScore(data) {
  const scoreEl = document.getElementById('completion-score');
  if (scoreEl) scoreEl.textContent = `${data?.score ?? currentLabState?.score ?? 0} / ${data?.max_score ?? currentLabState?.max_score ?? 100}`;
}
...
async function submitFlag() {
  ...
  try {
    ...
    feedback.textContent = data.correct
      ? '✅ Correct! You retrieved the exposed sensitive file and confirmed the information disclosure.'
...
async function submitKnowledgeCheck() {
  ...
  try {
    const res = await fetch('/api/knowledge-check', {
      ...
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        disclosure_answer: document.getElementById('kc-disclosure').value,
        control_answer: document.getElementById('kc-control').value,
    ...
    if (data.passed) {
      ...
      if (!data.disclosure_ok) msg += 'Reconsider what information disclosure means (exposing sensitive data). ';
      if (!data.control_ok) msg += 'Reconsider the control (authorization / access control on files). ';
      if (!data.summary_ok) msg += 'Add more detail to the summary. ';
      if (!data.flag_captured) msg += 'Retrieve the exposed file and capture the flag before the final check.';
